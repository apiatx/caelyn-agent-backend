...
async def _master_screener_loop():
    ...
    Architecture:
      ...
      - Inter-cycle sleep: 60s — preserves shared provider/runtime headroom.
    ...
    Expected cycle times:
      ...
      Period (hot): 34s cycle + 60s sleep ≈ 94s between starts.
    """
    ...
    def _import_master_screener_dependencies():
    ...
    while True:
        ...
        try:
            ...
            try:
                def _enrich_master_rows(rows):
                    from data.options_enricher import enrich_ticker_rows
                    enrich_ticker_rows(rows)

                await asyncio.to_thread(
                    _enrich_master_rows,
                    screener_data.get("tickers", []),
                )
            ...
            await asyncio.to_thread(_save_master_lkg_to_disk, full_result)
...
# ═══════════════════════════════════════════════════════════════════════════════
# Canonical computation, cache, and score helpers moved to:
#   backend/services/trading_dashboard_service.py
...
# All score helpers and _compute_dashboard moved to:
#   backend/services/trading_dashboard_service.py → compute_trading_dashboard()
#
# Legacy wrapper preserved for any internal callers that might reference the
# old function name — delegates to the canonical service.
def _compute_dashboard(
    ...
    from services.trading_dashboard_service import compute_trading_dashboard
    return compute_trading_dashboard(
        mode=mode,
        risk_data=risk_data,
...
async def trading_dashboard_refresh(
    ...
    from services.trading_dashboard_service import clear_dashboard_cache
