...
def _validate_basket_hashes(
    ...
    *,
    ...
    for row in themes:
        ...
        # label changes are reflected immediately without a cache bust or LKG rewrite.
        live_display = meta.get("display_name") or row.get("display_name", "")

        # Canonical taxonomy fields — always refreshed from the live registry so
        # reclassification, parent_theme_id changes, rollup updates, and
        # promoted/demoted nodes surface immediately without a full LKG rewrite.
        _live_classification = meta.get("classification") or row.get("classification", "")
        _live_parent_sector  = meta.get("parent_sector")
        _live_parent_tid     = meta.get("parent_theme_id")
        _live_rollup         = _get_effective_rollup(tid, THEME_RS_UNIVERSE)
        # assignable is True by default; market_lens and deprecated nodes carry False.
        _live_assignable     = meta.get("assignable", True)

        # Common repair fields applied in all three branches (legacy / stale / current)
        _taxonomy_patch: dict = {
            "display_name":      live_display,
            "classification":    _live_classification,
            "parent_sector":     _live_parent_sector,
            "parent_theme_id":   _live_parent_tid,
            "rollup_sector_ids": _live_rollup,
            "assignable":        _live_assignable,
        }

        if stored_hash is None:
            # Old-format row — no hash stored (pre-membership-hash LKG snapshot).
            # Cannot verify proxy_symbols currency; override membership fields
            # from ENRICHED_THEME_RS_UNIVERSE so admin adds/removes are reflected
            # immediately even from old LKG rows.
            legacy_count += 1
            patched_rows.append({
                **row,
                **_taxonomy_patch,
            ...
            # Do NOT serve stale curve data; return empty so the frontend shows
            # a warming/pending state instead of a wrong-membership chart.
            stale_count += 1
            patched_rows.append({
                **row,
                **_taxonomy_patch,
            ...
            # Hash matches — basket is current-membership; still refresh taxonomy.
            patched_rows.append({
                **row,
                **_taxonomy_patch,
